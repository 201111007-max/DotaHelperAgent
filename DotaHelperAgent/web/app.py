...
from utils.prompt_strategy import LangfusePromptStrategy, LocalYAMLPromptStrategy
...
prompt_manager = None  # 全局 Prompt 管理器实例
...
def initialize_prompt_manager() -> PromptManager:
    ...
    支持 Langfuse 远程存储和本地 YAML 降级方案
    
    Returns:
        PromptManager: 初始化完成的 Prompt 管理器实例
    """
    ...
    try:
        ...
        if LANGFUSE_AVAILABLE and langfuse_client:
            try:
                primary_strategy = LangfusePromptStrategy(langfuse_client)
                app_logger.info("Prompt 管理器主策略：Langfuse")
            except Exception as e:
                app_logger.warning(f"Langfuse 策略初始化失败: {e}")
        
        # 初始化本地 YAML 降级策略
        fallback_strategy = LocalYAMLPromptStrategy(
            prompts_dir=str(project_root / "config" / "prompts")
        )
        app_logger.info("Prompt 管理器降级策略：本地 YAML")
        ...
        app_logger.info("Prompt 管理器初始化完成")
        return prompt_manager
        
    except Exception as e:
        app_logger.error(f"Prompt 管理器初始化失败: {e}")
        # 返回一个仅使用本地策略的降级版本
        fallback_strategy = LocalYAMLPromptStrategy(
            prompts_dir=str(project_root / "config" / "prompts")
        )
