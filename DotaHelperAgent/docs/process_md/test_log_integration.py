...
class TestLogAPIIntegration:
    ...
    def client_and_handler(self, temp_log_dir):
        """创建测试客户端和处理器"""
        # 重置内存处理器以确保干净的测试环境
        from utils.memory_log_handler import reset_memory_handler, get_memory_handler
        reset_memory_handler()
        
        with patch('utils.log_config.LOG_DIR', Path(temp_log_dir)):
            # 重新获取新的 memory_handler 实例
            new_handler = get_memory_handler(2000)
            
            # 重新导入 app 并更新其 memory_handler 引用
            import importlib
            import web.app as app_module
            importlib.reload(app_module)
            
            # 更新 app 模块的 memory_handler 引用
            app_module.memory_handler = new_handler
