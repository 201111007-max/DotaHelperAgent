...
class MemoryLogHandler(logging.Handler):
    ...
    def _get_min_valid_idx(self) -> int:
        """计算最小有效索引"""
        return self._log_counter - len(self._logs)
    
    def _cleanup_invalid_indices(self):
        """清理失效的索引"""
        min_valid_idx = self._get_min_valid_idx()
        
        # 清理 trace_index
        for trace_id in list(self._trace_index.keys()):
            valid_indices = [
                idx for idx in self._trace_index[trace_id] 
                if idx >= min_valid_idx
            ]
            if valid_indices:
                self._trace_index[trace_id] = valid_indices
            else:
                del self._trace_index[trace_id]
        
        # 清理 error_index
        self._error_index = [
            idx for idx in self._error_index 
            if idx >= min_valid_idx
        ]
    ...
    def get_trace_logs(self, trace_id: str) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            for idx in indices:
                if idx >= min_valid_idx:
                    deque_idx = idx - min_valid_idx
                    try:
                        logs.append(self._logs[deque_idx])
                    except IndexError:
                        pass
        ...
        with self._lock:
            ...
            for idx in recent_error_indices:
                if idx >= min_valid_idx:
                    deque_idx = idx - min_valid_idx
                    try:
                        errors.append(self._logs[deque_idx])
                    except IndexError:
                        pass
