...
基于 ReAct Agent 和 OpenDota API 的 Dota 2 智能助手，提供英雄推荐、阵容分析、出装建议等全能辅助功能。采用 **ReAct 推理循环 + 三层记忆系统** 架构，支持多轮对话上下文理解。

## ✨ 核心特性

- ⚔️ **英雄推荐** - 根据双方阵容智能推荐最佳英雄
- 🎯 **阵容分析** - 敌我双方阵容综合评估与克制分析
- 🎒 **出装推荐** - 核心装备、针对出装、局势出装建议
- 📚 **技能加点** - 技能升级顺序与天赋树推荐
- 💬 **多轮对话** - 上下文理解、指代消解、智能问答
- 🧠 **ReAct Agent** - Think→Plan→Execute→Observe→Reflect 推理循环
- 🏗️ **10+ Tools** - 英雄分析、物品推荐、数据查询等工具集合
- 📝 **三层记忆** - 短期/长期/情景记忆系统（SQLite）
...
└─────────────────────────────────────────────────────────────┘
     ...
     └────┬────┘
          ...
          ▼
     ┌─────────┐
     │ Execute │  ← 执行工具调用（10+ Tools）
     └────┬────┘
          │
          ▼
     ┌─────────┐
     │Observe  │  ← 观察工具返回结果，收集证据
     └────┬────┘
          │
          ▼
     ┌─────────┐
     │ Reflect │  ← 反思：是否需要调整计划？是否遗漏信息？
     └────┬────┘
          │
          ▼
     ┌───────────────────┐
     │   Synthesize      │  ← 综合所有信息，生成最终答案
     └───────────────────┘
```
...
**注意**: 如果 `npm` 命令不可用，可能是 Node.js 未添加到系统 PATH。可以：
...
2. 或将 Node.js 添加到环境变量：
   ```powershell
   [Environment]::SetEnvironmentVariable("Path", $env:Path + ";D:\Scoop\apps\nodejs\current", "User")
   ```
